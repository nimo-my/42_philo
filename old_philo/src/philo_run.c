...
/*   Updated: 2022/08/12 16:34:44 by jisookim         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
int	philo_run(t_philo *philo)
{
	...
	while (1)
	{
		if (philo_eat(philo, &flag) == RET_ERROR)
			break ;
		if (philo->info->finish_flg == philo->info->num_philo)
		{
		...
		if (!philo_sleep(philo))
			break ;
		if (philo->info->argc_flg && philo->eat_count == philo->info->num_must_eat)
			return (OK);
	}
	return (PHIL_DIE);
}
...
int take_fork_and_eat(t_philo *philo, int first_fork, int second_fork)
{
	...
	pthread_mutex_lock(&(philo->info->m_fork[first_fork]));
	pthread_mutex_lock(&(philo->info->m_fork[second_fork]));
...
{
	usleep(philo->info->time_to_eat); // eat
...
{
	pthread_mutex_unlock(&(philo->info->m_fork[second_fork]));
	pthread_mutex_unlock(&(philo->info->m_fork[first_fork]));
	philo->time->eat_time = set_time(philo->time->eat) - (philo->time->start_time); // set time
	philo->eat_count++;
	if (philo->eat_count == philo->info->num_must_eat)
		philo->info->finish_flg++;
...
int philo_sleep(t_philo *philo)
{
	...
	if (philo->time->now_time > philo->info->time_to_die)
		return (PHIL_DIE);
	return (OK);
}
...
// 	while (philo->eat_count < philo->info->num_must_eat)
// 	{
...
// 		if (philo->flg_think || philo->flg_eat || philo->flg_sleep || (philo->time->living_time[2] >= (unsigned long long)philo->info->time_to_die))
// 		{
...
// 	if (philo->time->eat_time[1] - philo->time->eat_time[0] > philo->info->time_to_die) // starving : if check_start - check_end > starve_time
// 	{
// 		printf("[] philo : [%d], starve time : [%ld]\n ",philo->id, philo->time->eat_time[1] - philo->time->eat_time[0]);
// 		return (PHIL_DIE);
// 	}
// 	pthread_mutex_unlock(&(philo->info->m_fork[philo->left_fork]));
// 	pthread_mutex_unlock(&(philo->info->m_fork[philo->right_fork]));
// 	philo->eat_count++;
...
// {
// 	if (think_time[1] - think_time[0] > philo->info->time_to_think)
// 		return (PHIL_DIE);
// 	else
// 	{
